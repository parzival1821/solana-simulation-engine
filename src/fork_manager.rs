...
impl ForkManager {
    ...
    pub async fn set_token_balance(
        ...
        let token_account_pubkey = get_associated_token_address(
            &owner_pubkey,
            &mint_pubkey,
        );
...
mod tests {
    ...
    async fn test_fork_isolation() {
        ...
        manager.set_balance(&fork1, address, 5_000_000_000).await.unwrap();
        manager.set_balance(&fork2, address, 9_000_000_000).await.unwrap();
    ...
    async fn test_set_and_get_balance() {
        ...
        // Set balance
        manager.set_balance(&fork_id, address, expected_balance).await.unwrap();
        
        // Get balance
        let actual_balance = manager.get_balance(&fork_id, address).await.unwrap();
        
        assert_eq!(actual_balance, expected_balance);
        println!("✓ Balance set and retrieved correctly: {} lamports", actual_balance);
    }
    ...
    async fn test_multiple_accounts_in_fork() {
        ...
        manager.set_balance(&fork_id, addr1, 1_000_000_000).await.unwrap();
        manager.set_balance(&fork_id, addr2, 2_000_000_000).await.unwrap();
        manager.set_balance(&fork_id, addr3, 3_000_000_000).await.unwrap();
    ...
    async fn test_balance_update() {
        ...
        // Set initial balance
        manager.set_balance(&fork_id, address, 5_000_000_000).await.unwrap();
        let balance1 = manager.get_balance(&fork_id, address).await.unwrap();
        assert_eq!(balance1, 5_000_000_000);
        
        // Update balance
        manager.set_balance(&fork_id, address, 10_000_000_000).await.unwrap();
    ...
    async fn test_concurrent_fork_access() {
        ...
        let handle1 = tokio::spawn(async move {
            manager1.set_balance(&fork_id1, address1, 1_000_000_000).await.unwrap();
        });
        
        let manager2 = Arc::clone(&manager);
        let fork_id2 = fork_id.clone();
        let handle2 = tokio::spawn(async move {
            manager2.set_balance(&fork_id2, address2, 2_000_000_000).await.unwrap();
        });
    ...
    async fn test_spl_token_balance() {
        ...
        match manager.set_token_balance(&fork_id, owner, usdc_mint, amount).await {
            Ok(_) => {
                // Get token balance
                let balance = manager.get_token_balance(&fork_id, owner, usdc_mint)
                    .await
                    .unwrap();
    ...
    async fn test_fork_expiration_timestamp() {
        ...
        println!("✓ Fork created {} seconds ago", time_since_creation.as_secs());
